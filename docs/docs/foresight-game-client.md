...
### Published-scenario down-flow (implemented)

For a client display app that renders a project's **published scenarios** (not the
deck), the internal app exposes these read-only endpoints under the same base URL +
`X-Foresight-Key` auth. "Published" means a scenario is **approved and not
soft-deleted**; pending, rejected, and deleted scenarios (and soft-deleted sets)
never appear.

Fetch model is **list → detail**: lists return lightweight cards; only the
single-scenario endpoint returns the full body/gallery/links.

- **`GET /projects/:ref/scenario-sets`** -> `PublicScenarioSetSummary[]`. Lightweight
  summaries so the app can enumerate what's available. Only non-deleted sets with
  ≥1 published scenario are returned.
  ```jsonc
  { "id", "domain", "horizonYear", "format",
    "sharedUncertainties": [{ "axis", "outcomes", "resolution" }],
    "scenarioCount", "updatedAt" }
  ```
- **`GET /projects/:ref/scenario-sets/:setId`** -> one set with its scenarios as
  `PublicScenarioCard[]`. `404` if the set is deleted or has no published members.
- **`GET /projects/:ref/scenarios`** -> flat `PublicScenarioCard[]` across the project
  (optionally `?set_id=<uuid>`). Scenarios in a soft-deleted set are excluded.
- **`GET /projects/:ref/scenarios/:scenarioRef`** -> one **full** `PublicScenario`,
  resolved by `slug` (preferred, stable) or scenario `id` UUID.

`PublicScenarioCard` (list/grid shape): `id` (slug), `setId`, `position`, `title`,
`headline`, `teaser`, `theme`, `mood`, `icon`, `timeHorizon`, `coverImageUrl`
(first gallery image), `updatedAt`.

`PublicScenario` (full detail shape, camelCase):
```jsonc
{ "id",            // == the scenario slug (stable public ref)
  "setId", "title", "headline", "teaser",
  "theme": { "slug", "label" },
  "mood": { "label", "colorHex", "emotionalRegister" },
  "icon", "timeHorizon": { "year", "label" }, "format",
  "body",           // Markdown
  "openQuestion",
  "earlySignals": [{ "statement", "sources": [{ "url", "label" }] }],
  "images": [{ "url", "prompt", "position", "source" }],
  "linkedDrivers": [{ "driverId", "name" }],
  "linkedUncertainties": [{ "uncertaintyId", "title", "resolution" }],
  "sections": { /* interim rich house-style prose, keys not final */ },
  "updatedAt" }
```

**Image URL caveat:** `coverImageUrl` and `images[].url` are **signed URLs resolved at
read time and they expire**. The consuming app must re-fetch the scenario to refresh
them, not cache the URL long-term.

A consumer-facing version of this contract (with full response examples and fetch
code) lives at [`scenario-display-api.md`](scenario-display-api.md) for handoff.
